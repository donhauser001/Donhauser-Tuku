...
<?php if (!empty($albums_array)): ?>
    <div class="my-gallery-container">
        <h2><?php echo esc_html($category_name); ?>分类素材集</h2>
        <p><?php echo esc_html($category_name); ?>分类中有 <?php echo esc_html(count($albums_array)); ?> 个素材集</p>
    </div>
        ...
        <div class="dhs-album-grid">
            <?php foreach ($albums_array as $album): ?>
                <div class="dhs-album-item">
                    <a href="<?php echo esc_url(add_query_arg('album_id', $album['id'], site_url('/tuku/album-details/'))); ?>"> <img src="<?php echo esc_url($album['cover_image']); ?>" alt="<?php echo esc_attr($album['name']); ?>" />
                    </a>
